...
using rm = UsefulUtilities.Resources.UI.Distributions;
...
namespace UsefulUtilities.UI
{
    public partial class Distributions : System.Windows.Window
    {
        ...
        public Distributions(Func<List<DistributionSettings>> loadSettings, Action<List<DistributionSettings>> saveSettings)
        {
            ...
            {
                UsefulUtilities.Core.Message.Show(rm.failLoad + Environment.NewLine + ex.ToString());
            }
        }
        ...
        private void Dist_OnSave()
        {
            try
            {
                ...
                UsefulUtilities.Core.Message.Show(rm.distSettingsSaved);
            }
            catch (Exception ex)
            {
                UsefulUtilities.Core.Message.Show(rm.failSave + Environment.NewLine + ex.ToString());
            }
        }
